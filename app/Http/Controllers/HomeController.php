...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $user = DB::select( DB::raw("SELECT users.id as id,users.name as name, users.email as email, personal_data_of_users.image_path as image,personal_data_of_users.about_user as about_user ,personal_data_of_users.last_login as last_login  FROM users INNER JOIN personal_data_of_users ON users.id=personal_data_of_users.user_id where users.id=".$userId."") );
        // $checkAdmin =Admin::where("user_id", $userId)->get();
